...
### 移动端响应式审计与修复:座位图横向滚动提示 + 登录/注册切换链接触控热区(审计 2026-08-20 / 修复 2026-08-27,不算新 Phase)

这条记录本身在补一个文档缺口,不是先斩后奏——2026-08-20 那次移动端审计当时
只发布成了一个独立的 Artifact 链接(《CineVerse Mobile Audit》),从未正式写
进项目文档,直到这次准备合并修复分支时才发现"审计结论没有落进代码库"这件事
本身就是问题:过去每一次审计——2026-08-08 的"审计后修复"、2026-08-09 的
"顾客端交互反馈强化"——结论都写进了 CLAUDE.md,这一次是唯一的例外。下面是
那次审计的完整结论,加上这次针对两个真问题的修复。

**审计范围**:`app/(customer)/**` 下全部 9 个页面(`/`、`/login`、`/register`、
`/profile`、`/movies/[id]`、`/showtimes/[id]`、`/showtimes/[id]/seats`、
`/bookings`、`/bookings/[id]/confirmed`),每个页面在 375/390/414px 三个宽度
下各测一遍(iPhone SE / iPhone 12~14 / 较大 Android 这个区间)。座位图专门用
Hall 3 压力测试——3 个种子影厅里最大的一个(10 排 × 14 列,133 个可选座位,
对照 Hall 1 的 55、Hall 2 的 90,靠 `GET /halls/{id}/seats` 核实过,不是凭
`totalRows×totalColumns` 假设的)。需要登录态的页面(profile、订单列表、
电子票)用一个自建测试账号 + 一笔直接 SQL 改成 CONFIRMED 的订单(绕开
Stripe)——和这个项目一贯的 E2E 验证方式同一个模式,测试数据和临时实例
事后全部清理。

**两个真问题,这次已经修复**:

- **座位图横向滚动没有任何视觉提示**:Hall 3 的座位网格实际宽度 768px,而
  容器在三个测试宽度下只有 343~382px,三分之一到一半的座位在屏幕外,且没有
  淡出、图标或文字提示"这边还有座位"。**修复**:`components/booking/
  seat-map.tsx` 新增 `SCROLL_SHADOW_STYLE`——四层背景渐变叠加(两层
  `background-attachment: local` 的"遮盖层"贴着内容真正的首尾位置随内容
  滚动、两层 `scroll`-attachment 的琥珀色光晕钉死在容器可视区域的左右边缘),
  遮盖层只在"这一侧确实没有更多内容"时才会盖住光晕——不需要任何 JS 监听
  滚动位置,光晕会随着真实滚动状态自动出现/消失。
- **登录/注册页底部的切换链接是一个 17px 高的点击区域**:`login/page.tsx` 的
  "Sign up" 和 `register/page.tsx` 的"Log in"都是裸的 `<Link>`,没有任何
  尺寸规则,量出来只有 17px 高——不仅低于这个项目自己的 44×44 标准,连
  WCAG AA 更宽松的 24×24 下限都不够。**修复**:两处都改用
  `buttonVariants({variant: "ghost", size: "default"})`
  (`components/ui/button.tsx` 已有的 `h-11` 尺寸变体)——和 `navbar.tsx`
  自己的登录/注册链接是同一套尺寸系统,不是另起一套样式处理;外层从纯
  文本行改成 flex 行,让加高后的链接和旁边的问句文字居中对齐,不会把整段
  文字的行高意外撑高。

**审计确认完全没问题、任何宽度都不用改的页面**:`/movies/[id]`、
`/showtimes/[id]`、`/bookings`。

**审计顺手验证过的九项(均实测,不是假设,其中两项一开始像是问题、复核后
排除)**:
1. 全部 9 页 × 3 宽度(27/27)`scrollWidth` 与 `innerWidth` 完全一致,没有
   一个页面产生页面级横向滚动条。
2. Hall 3 全部 133 个座位按钮用 `getBoundingClientRect()` 实测,三个宽度下
   都是 44×44——不是只看源码里的 `h-11` 类名。
3. 移动端汉堡菜单本身工作正常(44×44 切换按钮,面板展开后链接尺寸也达标)
   ——第一轮自动化检查曾经因为 DOM 查询漏了一个兄弟元素而误报,已对照真实
   渲染截图纠正。
4. 电子票二维码固定 180×180px,三个宽度下都有 81~101px 的安全边距,不会
   被裁切。
5. 登录/注册表单本身(输入框、标签)三个宽度下都没有重叠或被裁切。
6. 首页电影网格在三个宽度下都稳定保持两列(154px 卡片 + 20px 间距)。
7. Hero 标题和轮播控件不会互相遮挡;上一页/下一页箭头在 `sm:` 断点以下
   正确隐藏,不会挤在窄屏上。
8. `/profile` 页"Log out"按钮量出来只有 28px 高——**不是新问题**:溯源到
   共享的 `LogoutButton` 组件默认 `size="sm"`(`logout-button.tsx:54`),
   正是 1.5 节里"导航栏登出按钮"这条已经写明的既有例外,自动化检查一开始
   标记了它,人工复核后确认不算新发现。
9. 某张截图里座位网格上方"浮着"一根条——溯源到 `pb-32` 这个专门为
   `SelectionSummaryBar`(fixed 定位)预留空间的 padding,是长截图拼接的
   视觉 artifact,真实用户永远不会看到,不算 bug。

**看到但没有计分的一项**:导航栏"CineVerse"品牌 wordmark 链接量出来
122×28px,高度同样不到 44px,而且和上面两处一样是裸 `<Link>` 而不是走
`Button` 尺寸系统——**没有**算进这次的两个真问题里:它是每个页面共用的
持久化 chrome(一个导航栏级别的决定,不是九个页面各自的问题),站点 logo
在业内的点击热区审计里几乎普遍被豁免,误触的代价也只是多点一次回首页。
值得知道,但优先级明显低于上面两个真问题,这次没有顺手改。

**审计过程中在范围之外发现的一个环境问题**:另起一个独立后端实例、连接
共享的开发数据库(`cineverse-postgres`)时,Flyway 在启动阶段直接拒绝
连接——`V2` 迁移的 checksum 和 `flyway_schema_history` 表里记录的不一致。
没有对着这个其他 session 可能还依赖的共享实例贸然跑 `flyway repair`,审计
改用完全一次性的 Postgres/Redis 容器完成验证,没有真的碰这个 mismatch——
已经补记进 `docs/DEVELOPMENT.md`「本机环境注意事项」,这里不重复内容。

**这次(2026-08-27)修复的验证方式**:另起一对临时后端(8090)/前端(3002)
实例,指向同一套共享 Postgres/Redis;用自建自清理的一场测试场次(Hall 3、
133 座位)和两个自建自清理的测试用户,跑真实 Playwright(chromium)在
375/390/414px 三个宽度下逐项复核——两个链接的点击区域实测 ≥44×44、座位图
容器确实溢出且 scroll-shadow 的 `background-attachment: local, local,
scroll, scroll` 计算样式确实生效、滚动到底部时光晕确实换边、座位选择/
取消选择逻辑不受影响、注册→登录完整表单提交流程不受影响。全程未触碰
3000/8081 上一直运行的开发实例(前后 PID 核对一致)。

完整的审计原始记录(逐项截图)见 Artifact「CineVerse Mobile Audit」
(2026-08-20 发布)。
