...
- **技术栈选了 Java 21 LTS + Spring Boot 3.5.15,而不是项目最初用的 Java 25 +
  ...
  供给的作品集项目,这个代价可以接受;完整的调研过程和取舍记录见
  `docs/DECISIONS.md`(`CLAUDE.md` 只保留结论)。
- **种子数据的电影海报/背景图来自 OMDb / TMDB,两者条款都禁止商业用途**——
  CineVerse 是非商业作品集项目,符合限制;但如果这个项目以后要商业化,这批
  图片数据源必须换掉。TMDB 要求的可见署名(官方 logo + 指定文案)已于
  2026-08-14 实现(顾客端 + admin 端 footer 的 `TmdbAttribution` 组件)。
...
```
CineVerse/
├── CLAUDE.md                 # 项目记忆:架构决策、路线图、当前生效的技术选型
├── docs/DECISIONS.md         # 决策与历史记录:被 CLAUDE.md 精简掉的完整论述、调试过程、已被取代的历史决定
...
哪些坑)可以看 [`CLAUDE.md`](./CLAUDE.md)(当前架构与结论)和
[`docs/DECISIONS.md`](./docs/DECISIONS.md)(完整论述过程、调试插曲、已被
取代的历史决定)——两份文档合起来记录了每一步架构选型和取舍的原因,是给
愿意刨根问底的读者(比如面试官)准备的加分材料,**不是理解这个项目必须先
读完的前提**。
